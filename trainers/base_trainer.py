...
class Trainer():
    ...
    def train(self) -> Dict:
        ...
        for epoch in range(self.start_round, self.global_rounds):
            ...
            # To Do: Local delta를 암호화하여 aggregate 함수에 전송

            # To Do: Local delta의 암호문을 활용하도록 server 단의 aggregate 함수 수정
            # 단, Global_state_dict는 현재 round 수 t가 1 이상이면 암호문 상태로 존재
            # Server-side
            ...
            updated_global_state_dict, server_stats = self.server.encrypted_aggregate(local_weights, local_deltas,
                                                              selected_client_ids, copy.deepcopy(global_state_dict), current_lr)
            ...
            if self.args.eval.freq > 0 and epoch % self.args.eval.freq == 0:
                eval_results = self.evaluate(epoch=epoch, local_datasets=local_datasets)
            ...
            wandb_dict.update({
                # delta
                "server/delta_mean": server_stats["delta"]["mean"],
                "server/delta_std": server_stats["delta"]["std"],
                "server/delta_min": server_stats["delta"]["min"],
                "server/delta_max": server_stats["delta"]["max"],
                "server/delta_median": server_stats["delta"]["median"],
                "server/delta_skew": server_stats["delta"]["skew"],

                # momentum (m)
                "server/m_mean": server_stats["m"]["mean"],
                "server/m_std": server_stats["m"]["std"],
                "server/m_min": server_stats["m"]["min"],
                "server/m_max": server_stats["m"]["max"],
                "server/m_median": server_stats["m"]["median"],
                "server/m_skew": server_stats["m"]["skew"],

                # v
                "server/v_mean": server_stats["v"]["mean"],
                "server/v_std": server_stats["v"]["std"],
                "server/v_min": server_stats["v"]["min"],
                "server/v_max": server_stats["v"]["max"],
                "server/v_median": server_stats["v"]["median"],
                "server/v_skew": server_stats["v"]["skew"],
            })
